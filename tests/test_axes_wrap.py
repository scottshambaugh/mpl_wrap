...
def test_axeswrap_methods_delegate() -> None:
    ...
    ax.hlines_wrapped([355.0], [0.0], [10.0])
    ax.vlines_wrapped([5.0], [350.0], [370.0])
    ax.axhspan_wrapped(340.0, 380.0, alpha=0.2)
    ax.axvspan_wrapped(1.0, 2.0, alpha=0.2)
    ax.fill_between_wrapped(x, 90.0 * x, 110.0 * x, alpha=0.3)
    ax.fill_betweenx_wrapped(x, 90.0 * x, 110.0 * x, alpha=0.3)
    ax.step_wrapped(x, 100.0 * x, where="mid")
    ax.stairs_wrapped(100.0 * x[:-1], x)
    ax.stairs_wrapped(100.0 * x[:-1], x, fill=True, alpha=0.3)
